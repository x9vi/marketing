...
function sendAuthUser(user: { id: string; username: string; email?: string | null; name: string; role: Role; active?: boolean }) {
  return {
    id: user.id,
...
app.get('/reports/sales-detail', authRequired, requireRole(Role.ADMIN), async (req, res, next) => {
  try {
    ...
    const pm = typeof req.query.paymentMethod === 'string' && req.query.paymentMethod ? req.query.paymentMethod as PaymentMethod : undefined;
    ...
    for (const s of chartSales) { const d = format(s.createdAt, 'yyyy-MM-dd'); dailyMap[d] = (dailyMap[d] || 0) + toNumber(s.total); }
...
app.get('/reports/payment-methods', authRequired, requireRole(Role.ADMIN), async (req, res, next) => {
  try {
    ...
    const bm: Record<string, { count: number; total: number }> = {};
    for (const p of payments) { if (!bm[p.method]) bm[p.method] = { count: 0, total: 0 }; bm[p.method].count++; bm[p.method].total += toNumber(p.amount); }
...
app.get('/reports/employees', authRequired, requireRole(Role.ADMIN), async (req, res, next) => {
  try {
    ...
    const sm: Record<string, { txns: number; rev: number; disc: number; ref: number; voids: number }> = {};
    for (const u of users) sm[u.id] = { txns: 0, rev: 0, disc: 0, ref: 0, voids: 0 };
    for (const s of salesData) { if (!sm[s.userId]) sm[s.userId] = { txns: 0, rev: 0, disc: 0, ref: 0, voids: 0 }; sm[s.userId].txns++; sm[s.userId].rev += toNumber(s.total); sm[s.userId].disc += toNumber(s.discountAmount) + toNumber(s.couponDiscount); }
    for (const r of refundsData) { if (!sm[r.createdById]) sm[r.createdById] = { txns: 0, rev: 0, disc: 0, ref: 0, voids: 0 }; sm[r.createdById].ref += toNumber(r.amount); }
    for (const v of voidsData) { if (!sm[v.userId]) sm[v.userId] = { txns: 0, rev: 0, disc: 0, ref: 0, voids: 0 }; sm[v.userId].voids++; }
...
app.get('/reports/customers-report', authRequired, requireRole(Role.ADMIN), async (req, res, next) => {
  try {
    ...
    const spend: Record<string, number> = {}, txns: Record<string, number> = {};
    for (const s of salesInRange) { if (!s.customerId) continue; spend[s.customerId] = (spend[s.customerId] || 0) + toNumber(s.total); txns[s.customerId] = (txns[s.customerId] || 0) + 1; }
...
app.get('/reports/tax', authRequired, requireRole(Role.ADMIN), async (req, res, next) => {
  try {
    ...
    const dailyMap: Record<string, { tax: number; revenue: number }> = {};
    for (const s of sales) { const d = format(s.createdAt, 'yyyy-MM-dd'); if (!dailyMap[d]) dailyMap[d] = { tax: 0, revenue: 0 }; dailyMap[d].tax += toNumber(s.taxAmount); dailyMap[d].revenue += toNumber(s.total); }
...
type AppSettings = typeof defaultSystemSettings;
...
async function writeSettings(data: AppSettings): Promise<AppSettings> {
  const record = await prisma.appSetting.upsert({
    where: { key: settingsKey },
    update: { value: data as unknown as Prisma.InputJsonValue },
    create: { key: settingsKey, value: data as unknown as Prisma.InputJsonValue }
  });
  return record.value as AppSettings;
}
...
app.put('/settings', authRequired, requireRole(Role.ADMIN), async (req: AuthedRequest, res, next) => {
  try {
...
// ────────────────────────────────────────────────
app.post('/cashier/drawer/pop', authRequired, async (req: AuthedRequest, res, next) => {
  try {
    const { reason } = req.body as { reason?: string };
    const stationId = (req.headers['x-station-id'] as string | undefined) ?? 'unknown';
    ...
    await logActivity(
      ...
      'cashDrawer',
      drawer?.id,
      { reason: reason ?? 'Manual open', stationId, manual: true, userName: req.user?.name } as Prisma.InputJsonValue
    );
